...
import {
    mapboxDirectionStyle
} from './mapboxDirection.js';
import {
    RouteLocations
} from '/imports/api/routeLocations/routeLocations.js';
...
Template.mapbox.onCreated(function() {
    ...
    if (Meteor.settings.public.isLocal) //for testing purpose only
        subdomain = ['doublebay'];
    Logger.log({
        action: `${Meteor.settings.public.userAppActions.mapRender} ${subdomain[0]}`
    });
...
Template.mapbox.onRendered(function() {
    ...
    Meteor.setTimeout(() => {
        var subdomain = document.location.hostname.split('.');
        Logger.log({
            action: `${Meteor.settings.public.userAppActions.mapRendered} ${subdomain[0]}`
        });
...
Template.mapbox.events({
    ...
    'click .marker' (event) {
        ...
        Logger.log({
            action: `${Meteor.settings.public.userAppActions.markerPressed}, name: ${markerDetails[0].name}`
        });
    },
    ...
    'click .more-info' (event, inst) {
        ...
        Logger.log({
            action: `${Meteor.settings.public.userAppActions.detailsButtonPressed}, name: ${markerDetails[0].name}`
        });
    ...
    'click .directions' (event, inst) {
        ...
        Logger.log({
            action: `${Meteor.settings.public.userAppActions.directionsButtonPressed}, name: ${markerDetails[0].name}`
        });
