    ...
    'submit #newEvent': (event, inst) => {
        ...
        try {
            const newEventObject = {
                ...
                from: $('#eventFrom').pickadate().pickadate('picker').get('select').obj,
            ...
            uploadFile('eventimage1', 'image1', inst).then(function(imgPath){
                ...
                Meteor.call('EventDev.insert', newEventObject, $('#eventId').val(), (error, result) => {
                    if (error) {
                        Session.set('showLoading',false);
                        Bert.alert({
                            title: 'Ups...',
                        ...
                        Meteor.call('menuItemDev.addEvent', event, (error, result) => {
                            if (error) {
                                Session.set('showLoading',false);
    ...
    'submit #newLocation': (event, inst) => {
        ...
        try {
            ...
            uploadFile('locationImage1', 'locationImage1', inst).then(function(imgPath){
                ...
                Meteor.call('LocationData.insert', newLocationObject, $('#sensisId').val(), $('#locationRefId').val(), (error, result) => {
                    if (error) {
                        Session.set('showLoading',false);
                        Bert.alert({
                            title: 'Ups...',
                        ...
                        Meteor.call('LocationDev.insert', locationDevObject, $('#locationDevId').val(), (error, result) => {
                            if (error) {
                                Session.set('showLoading',false);
                                Bert.alert({
                                    title: 'Ups...',
                                ...
                                Meteor.call('menuItemDev.addLocation', locationInfo, (error, result) => {
                                    if (error) {
                                        Session.set('showLoading',false);
                                        throw Meteor.Error(Number, error);
                                    } else {
...
function autoPopulateEventFields(event) {
    ...
    if (event.begin) {
        $('#eventFrom').pickadate().pickadate('picker').set('select', new Date(event.begin))
