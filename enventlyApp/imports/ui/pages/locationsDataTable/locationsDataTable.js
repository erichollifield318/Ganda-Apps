...
function performSorting(locations, key) {
  ...
  if (locations && locations.length) {
    ...
    if (sortDataset[0]==='name') {
      ...
      result = _.sortBy(locations, function(location) {
        return getMenuSubmenuNames(location).subMenuName.trim().toLowerCase();
      });
    } else if(sortDataset[0]=='menu') {
      result = _.sortBy(locations, function(location) {
        return getMenuSubmenuNames(location).mainMenu.trim().toLowerCase();
      });
    }
...
function getMenuSubmenuNames(location) {
  ...
  if(getSubdomain(getCookie("selectedSDForSA")))
  {
    menu = dynamicCollections[getSubdomain(getCookie("selectedSDForSA"))+'_menu_item_dev'].find({'items.custom_locations': { $in: [location._id] }}).fetch();
  }
  else {
  ...
  if(menu && menu.length){
    menu.forEach((m, ind)=>{
      if(ind==0) mainMenu += m.name
      else mainMenu = mainMenu+', '+m.name

      m.items.forEach((i, ind2)=>{
        if(i.custom_locations.includes(location._id)) {
          subMenuName += i.name+', ';
        }
      })
      subMenuName = subMenuName.substring(0, subMenuName.length - 2);
    })
  }
