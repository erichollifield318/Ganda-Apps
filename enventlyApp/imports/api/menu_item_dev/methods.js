...
Meteor.methods({
    ...
    'menuItemDev.remove' (deleteObject,sd) {
        if(getSubdomain(sd))
        {
          dynamicCollections[getSubdomain(sd)+'_menu_item_dev'].update({ _id: deleteObject.menuId }, { $pull: { items: { name: deleteObject.name } } }, { multi: true });
        }
        else {
          MenuItemDev.update({ _id: deleteObject.menuId }, { $pull: { items: { name: deleteObject.name } } }, { multi: true });
        }
    },
    ...
    'menuItemDev.upsertCustomIndex'(sd){
        ...
        if(getSubdomain(sd))
        {
          console.log('getSubdomain(sd)',getSubdomain(sd))
...
function getHighestCustomIndex(selectedMenuId, itemInfo,sd){
    var highestCustomIndexItem;
          ...
          highestCustomIndexItem =  dynamicCollections[getSubdomain(sd)+'_menu_item_dev'].aggregate([
                 { $match: { _id: selectedMenuId } },
                  { $unwind : "$items" },
                  { $sort : {"items.customIndex":-1} },
                  { $limit :1}
                  ])
          }
          else {
