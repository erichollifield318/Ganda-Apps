...
Meteor.methods({
  'custom_location.findLocationDev'(locationName) {
    Logger.log({action: `${Meteor.settings.public.userAppActions.localQuery} ${JSON.stringify({name:locationName})}`});
    var locationInfo;
    if(getSubdomain())
    {
    ...
    Logger.log({action: `${Meteor.settings.public.userAppActions.localResult} ${LocationDev.find({name:locationName}).fetch().length}`});
    return locationInfo;
  },
