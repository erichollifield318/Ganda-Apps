...
Template.editableMapbox.onRendered(function() {
	...
	if(this.marker?this.marker.get():false){
        ...
        mapObj.zoom = Session.get('defaultZoomLevel');
        mapObj.center = Session.get('defaultCords');
...
Template.editableMapbox.helpers({
    latitude(){
        ...
        if(inst.mapMarker && inst.mapMarker.get()){
            return inst.mapMarker.get()._lngLat.lat;
        }
    },
    longitude(){
        let inst = Template.instance()
        if (inst.mapMarker && inst.mapMarker.get()){
            return inst.mapMarker.get()._lngLat.lng;
        }
    }
});

Template.editableMapbox.events({
    'click .modal-action': function(e, i) {
        const marker = i.marker.get();
        if (marker && marker.newCoords && marker.newCoords.latitude && marker.newCoords.longitude) {
            const newCoords = {
                location: marker.name,
                newCoords: marker.newCoords,
                updatedDate: new Date(),
                currentLocationID: marker._id,
                currentSelectedSubdomain: getCookie('selectedSDForSA'),
            };

            Meteor.call('locationCoords.insert', newCoords, (err, result) => {
                if (err) {
                    console.log("Error in method call:(locationCoords.insert) ", err)
                }else {
