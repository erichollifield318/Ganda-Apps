...
Meteor.methods({
  'EventDev.insert'(newEvent, eventId, subdomain) {
    check(newEvent, Object);
    if(!eventId){
      newEvent["createdAt"] = new Date()
      if (getSubdomain(subdomain) && dynamicCollections[getSubdomain(subdomain)+'_event_dev']) {
        return dynamicCollections[getSubdomain(subdomain)+'_event_dev'].insert(newEvent);
      }
      ...
      newEvent["updatedAt"] = new Date()
         ...
         if (getSubdomain() && dynamicCollections[getSubdomain()+'_event_dev']) {
  ...
  'EventDev.update'(eventId, newEventObject, subdomain) {
    check(newEventObject, Object);
    if (getSubdomain(subdomain) && dynamicCollections[getSubdomain(subdomain)+'_event_dev']) {
      return dynamicCollections[getSubdomain(subdomain)+'_event_dev'].update({ '_id': eventId},
        { $set: newEventObject });
    }
  ...
  'events.search' (eventName, userId) {
    ...
    Logger.log({action: `${Meteor.settings.public.userAppActions.localQuery}`, context: `${JSON.stringify({name:eventName})}`});
    Logger.log({action: `${Meteor.settings.public.userAppActions.localResult}`, context: `${EventDev.find({name:eventName}).fetch().length}`});
