...
Meteor.methods({
    ...
    'CacheDev.remove' (filter,sd) {
        var removed;
        if (getSubdomain(sd) && dynamicCollections[getSubdomain(sd)+'_cache_dev']) {
...
function findSensisLocation(searchObject) {
    ...
    Logger.log({action: `${Meteor.settings.public.userAppActions.sensisResult}`, context: `${data.results.length}`});
