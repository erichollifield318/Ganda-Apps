...
Meteor.methods({
  ...
  'getAllLocations' (sd) {
    this.unblock();
    let subdomain = getSubdomain(sd);
    // Get Location Records
    let locationDevRecords;
    if ( subdomain && dynamicCollections [ subdomain + '_location_dev' ]) {
      locationDevRecords =  dynamicCollections [ subdomain +'_location_dev'].aggregate([
        {
          $lookup:{
            from: subdomain+"_location_data",
        ...
        {
          $lookup:{
            from:subdomain+"_custom_location",
            localField:"location_ref_id",
            foreignField:"_id",
            as:"custom_location"
          }
	...
	'deleteBusinessLocation' (location) {
      ...
      if (getSubdomain() && dynamicCollections[getSubdomain()+'_location_data']) {
        ...
        locationDataStatus = LocationData.remove({ _id: location.location_ref_id });
  		...
  		let cacheDevStatus;
      if (getSubdomain() && dynamicCollections[getSubdomain()+'_cache_dev']) {
    ...
    if (dbLocations && dbLocations.length) {
      ...
      dbLocations.forEach((loc, i)=>{
        ...
        if (loc && loc.location_ref==='custom_location') {
          ...
          // console.log(":: locationDevStatus --- > ",locationDevStatus);
          let customLocationStatus;
            if (getSubdomain(sd) && dynamicCollections[getSubdomain(sd)+'_custom_location']) {
          ...
          // console.log(":: locationDevStatus --- > ",locationDevStatus);
          let locationDataStatus;
            if (getSubdomain(sd) && dynamicCollections[getSubdomain(sd)+'_location_data']) {
