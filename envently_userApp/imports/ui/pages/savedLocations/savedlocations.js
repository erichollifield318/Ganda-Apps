...
Template.savedlocations.onCreated(function() {
    ...
    Meteor.subscribe('admin_settings.all',getSubdomain(getCookie("selectedSDForSA")));
    Meteor.subscribe('usersdata.userLocations',getSubdomain(getCookie("selectedSDForSA")));
...
Template.savedlocations.helpers({
    locations() {
        ...
        var from = turf.point(currentLocation),
            ...
            {
              userDataList = dynamicCollections[getSubdomain(getCookie("selectedSDForSA"))+'_usersdata'].find({}).fetch()
            }
            else {
    ...
    count() {
        ...
        if(getSubdomain(getCookie("selectedSDForSA")))
        {
          count = dynamicCollections[getSubdomain(getCookie("selectedSDForSA"))+'_usersdata'].find({}).fetch()
        }
        else {
