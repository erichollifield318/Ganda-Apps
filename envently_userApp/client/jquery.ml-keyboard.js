...
import { searchQuery } from '/imports/ui/components/searchBar/searchBar.js';
...
Keyboard.prototype.showKeyboard = function($input) {
  ...
  $("#mapBox").css('height','100%');
  $(".page-footer").css({'bottom': '565px', 'transition': 'all .5s ease-out'});
  $("#sidebarBtn").css({'bottom':'655px', 'transition': 'all .5s ease-out'});
  $("#fabUserLocationBtn").css('bottom','566px');
  $("#markersBtn").css('bottom', '566px');
  $("#fabModalBtn").css('bottom','572px');

  var input_changed = !this.$current_input || $input[0] !== this.$current_input[0];

  setTimeout(() => {
    searchQuery.set(this.$current_input.val().trim());
  }, 300);
...
Keyboard.prototype.hideKeyboard = function() {
  if (this.options.is_hidden) {
    ...
    this.$current_input.focusout();
    $(".page-footer").css('bottom','0');
    $("#sidebarBtn").css('bottom','90px');
    $("#fabUserLocationBtn").css('bottom','0px');
    $("#markersBtn").css('bottom','0px');
    $("#fabModalBtn").css('bottom','6px');
    $("#mapBox").css('height','90%');
  }
};
